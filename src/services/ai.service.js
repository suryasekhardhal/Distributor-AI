...
import { buildOrderConfirmationMessage } from "./whatsappMessage.service.js";
...
export async function processDistributorMessage({
  ...
  const command = await parseDistributorMessage(message);

  command.customerPhone = customerPhone;
  ...
  if (
    ...
    const pending = await savePendingConfirmation({
      companyId,
